...
<div class="container bg-claro rounded-xl md:w-3/4 lg:w-3/5 md:p-10 p-14 mt-10 mb-24">
    ...
    <div class="grid md:grid-cols-2">
        ...
        <div class="flex flex-col gap-1">
            ...
            <nav><span class="font-bold">Material:</span> <span>{{ Auth::user()->nombre }}</span></nav>
            <nav><span class="font-bold">Relleno:</span> <span>{{ Auth::user()->apellido }}</span></nav>
            <nav><span class="font-bold">Calidad:</span> <span>{{ Auth::user()->email }}</span></nav>
            <nav><span class="font-bold">Tamaño:</span> <span>{{ Auth::user()->direccion }}</span></nav>
        </div>
    </div>
    ...
    <form id="payment-form" action="{{ route('pago') }}" method="post" class="flex flex-row  justify-center gap-4">
        @csrf
